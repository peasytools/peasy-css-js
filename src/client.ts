...
/** PeasyCss API client. Zero dependencies — uses native fetch. */
export class PeasyCss {
  private baseUrl: string;

  constructor(baseUrl = "https://peasycss.com") {
  ...
  private async get<T>(path: string, params?: Record<string, string | number | undefined> | object): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`PeasyCss API error: HTTP ${res.status} — ${body}`);
    }
